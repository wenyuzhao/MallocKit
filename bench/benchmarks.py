...
class cscratch(Benchmark):
    ...
    def run(self, env: List[str]):
        self.measure(f'./cache-scratch {PROCS} 1000 1 2000000  {PROCS}', env=env)
...
class gs(Benchmark):
    ...
    def run(self, env: List[str]):
        self.measure(f'gs -dBATCH -dNODISPLAY {MIMALLOC_BENCH_EXTERN_DIR}/large.pdf', env=env)
...
class lean(Benchmark):
    ...
    def run(self, env: List[str]):
        ...
        self.measure(f'../bin/lean --make -j {min(PROCS, 8)}', env=env, cwd=f'{MIMALLOC_BENCH_EXTERN_DIR}/lean/library')
