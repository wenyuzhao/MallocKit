...
parser.add_argument('-a', '--algorithms', nargs='*',
                    default=BENCHMARK_SUITE.algorithms)
parser.add_argument('-b', '--benchmarks', nargs='*',
                    default=[b.name for b in BENCHMARK_SUITE.benchmarks])
...
if args.build:
    ...
    subprocess.check_call(
        f'cargo build {flags} {features}', shell=True, text=True, cwd=cwd)
...
if args.record:
    assert len(
        args.algorithms) == 1, 'Only one malloc algorithm is allowed when specifying --record'
    assert len(
        args.benchmarks) == 1, 'Only one benchmark is allowed when specifying --record'
    assert args.invocations == 1, 'Only one invocation is allowed when specifying --record'
    assert args.events is None or len(
        args.events) <= 1, 'Only one perf-event is allowed when specifying --record'
