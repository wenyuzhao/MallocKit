...
pub trait Plan: GlobalAlloc {
    fn get_layout(&self, ptr: *mut u8) -> Layout;
}


pub struct MallocAPI<GA: Plan>(pub GA);
...
macro_rules! export_malloc_api {
    ($plan: expr) => {
        type Malloc = MallocAPI<impl Plan>;
        pub static MALLOC_IMPL: Malloc = $crate::MallocAPI($plan);
